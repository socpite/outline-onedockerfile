...
# Install core system packages in one layer
RUN apt-get update && apt-get install -y \
    curl wget ca-certificates gnupg lsb-release git vim nano htop tree jq unzip sudo \
    build-essential python3 python3-pip \
    xdotool imagemagick scrot wmctrl xclip \
...
# Install Redis, Nginx, and X11/VNC packages
RUN apt-get update && apt-get install -y \
    redis-server nginx \
    xvfb x11vnc websockify xfce4 xfce4-terminal firefox novnc gedit thunar \
    && rm -rf /var/lib/apt/lists/*
...
RUN adduser --disabled-password --gecos "" outline \
    ...
    && mkdir -p /var/lib/postgresql/16/main /var/log/postgresql /run/postgresql \
    && chown -R postgres:postgres /var/lib/postgresql /var/log/postgresql /run/postgresql \
    && mkdir -p /var/lib/redis /var/log/redis \
    && chown -R redis:redis /var/lib/redis /var/log/redis \
    && mkdir -p /opt/outline /var/lib/outline/data /var/log/outline /home/outline/screenshots /home/outline/.vnc \
    && chown -R outline:outline /opt/outline /var/lib/outline /var/log/outline /home/outline \
    && mkdir -p /etc/dinit.d /etc/dinit.d/boot.d /var/log/dinit

# Clone Outline source code (shallow clone for faster build)
WORKDIR /opt/outline
RUN git clone --depth 1 https://github.com/outline/outline.git . \
    && chown -R outline:outline . \

# Create all scripts in one layer
RUN cat > /usr/local/bin/setup_postgres_data.sh << 'EOF' && \
cat > /usr/local/bin/setup_postgres_env.sh << 'EOF2' && \
cat > /usr/local/bin/agent-screenshot << 'EOF3' && \
cat > /usr/local/bin/agent-rebuild << 'EOF4' && \
cat > /usr/local/bin/agent-restart << 'EOF5' && \
cat > /usr/local/bin/agent-status << 'EOF6' && \
cat > /usr/local/bin/agent-logs << 'EOF7' && \
cat > /usr/local/bin/agent-test << 'EOF8' && \
chmod +x /usr/local/bin/setup_postgres_data.sh /usr/local/bin/setup_postgres_env.sh /usr/local/bin/agent-*
#!/bin/bash
set -e
...
echo ""
echo "=== Network Status ==="
netstat -tlnp | grep -E "(3000|5432|6379|80)" 2>/dev/null || echo "netstat not available"
EOF6
#!/bin/bash
echo "=== Recent Outline Logs ==="
tail -50 /var/log/dinit/outline.log 2>/dev/null || echo "No outline logs yet"
echo ""
echo "=== Recent PostgreSQL Logs ==="
tail -20 /var/log/dinit/postgres.log 2>/dev/null || echo "No postgres logs yet"
EOF7
#!/bin/bash
cd /opt/outline
...
ln -sf /etc/nginx/sites-available/outline /etc/nginx/sites-enabled/default
...
# Create all dinit service files in one layer
RUN cat > /etc/dinit.d/boot << 'EOF' && \
cat > /etc/dinit.d/postgres-init << 'EOF2' && \
cat > /etc/dinit.d/postgres << 'EOF3' && \
cat > /etc/dinit.d/postgres-env << 'EOF4' && \
cat > /etc/dinit.d/redis << 'EOF5' && \
cat > /etc/dinit.d/xvfb << 'EOF6' && \
cat > /etc/dinit.d/xfce << 'EOF7' && \
cat > /etc/dinit.d/xfce.env << 'EOF8' && \
cat > /etc/dinit.d/x11vnc << 'EOF9' && \
cat > /etc/dinit.d/websockify << 'EOF10' && \
cat > /etc/dinit.d/nginx << 'EOF11' && \
cat > /etc/dinit.d/outline << 'EOF12' && \
cat > /etc/dinit.d/outline.env << 'EOF13' && \
ln -s /etc/dinit.d/postgres-init /etc/dinit.d/boot.d/ && \
...
# Set VNC password and create log files
RUN echo "outline" | sudo -u outline vncpasswd -f > /home/outline/.vnc/passwd \
    ...
    && touch /var/log/dinit/postgres.log /var/log/dinit/redis.log /var/log/dinit/xvfb.log \
             /var/log/dinit/xfce.log /var/log/dinit/x11vnc.log /var/log/dinit/websockify.log \
             /var/log/dinit/nginx.log /var/log/dinit/outline.log

# Create startup script
RUN cat > /usr/local/bin/container-start.sh << 'EOF' && chmod +x /usr/local/bin/container-start.sh
#!/bin/bash
set -e
